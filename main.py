...
def start_kb():
    return InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="💫 Давай", callback_data="start_form")]]
    )
...
def role_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🌱 Будущая мама / папа", callback_data="role_Будущий(ая)")]
    ])
...
def skip_about_kb():
    return InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="⏭ Пропустить", callback_data="skip_about")]]
    )
...
async def start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "«свойЧеловек» — это про тепло, поддержку\n"
        "и людей, с которыми можно быть настоящим.\n\n"
        "Начнём знакомство?",
...
async def set_city(message: Message, state: FSMContext):
    ...
    await message.answer(" ", reply_markup=role_kb())
...
async def send_my_profile(user_id: int):
    ...
    name, age, city, role, goal, about, photo_id = row
...
async def show_next_profile(call: CallbackQuery):
    ...
    uid, name, age, city, role, goal, about, photo_id = row
